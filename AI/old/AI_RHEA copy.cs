...
namespace SimpleWars
{
    ...
    {
        // Parameters for RHEA
        // private const int HORIZON = 8;                 // Length of the action sequence (rollout depth)
        // private const int POPULATION_SIZE = 15;        // Number of individuals in the population
        ...
        // private const double ATTACK_BIAS = 0.85;        // Bias towards attack actions (vs move actions)
        private const int HORIZON = 12;                // Increased from 8
        private const int POPULATION_SIZE = 30;        // Increased from 15
        private const int NUM_GENERATIONS = 15;        // Increased from 8
        private const double MUTATION_RATE = 0.4;      // Increased from 0.3
        private const double CROSSOVER_RATE = 0.8;     // Increased from 0.7
        private const int TOURNAMENT_SIZE = 5;         // Increased from 3
        private const long LIMIT_TIME = 9700;          // Kept the same
        private const int ELITISM_COUNT = 4;           // Increased from 2
        private const double ATTACK_BIAS = 0.8;        // Increased from 0.7
        ...
        private Individual Crossover(Individual parent1, Individual parent2)
        {
            ...
            // One-point crossover at a random horizon point
            int crossoverPoint = rnd.Next(1, HORIZON);
        ...
        private void Mutate(Individual individual, Map map, int teamColor)
        {
            ...
            for (int h = 0; h < HORIZON; h++)
            {
                ...
                if (rnd.NextDouble() < MUTATION_RATE)
                {
                    ...
                    double mutationType = rnd.NextDouble();
                    
                    if (mutationType < 0.4)
                    {
                        // Complete replacement (current implementation)
                        individual.actionSequence[h] = GenerateRandomActions(map, teamColor);
                    }
                    else if (mutationType < 0.7)
                    {
                        // Partial replacement - replace actions for a random subset of units
                        PartialActionReplacement(individual, h, map, teamColor);
                    }
                    else
                    {
                        // Action modification - modify parameters of existing actions
                        ModifyExistingActions(individual, h, map, teamColor);
                    }
                }
            }
        }
        
        // Partially replace actions for a random subset of units
        private void PartialActionReplacement(Individual individual, int horizon, Map map, int teamColor)
        {
            // If there are no actions at this horizon, generate new ones
            if (individual.actionSequence[horizon].Count == 0)
            {
                individual.actionSequence[horizon] = GenerateRandomActions(map, teamColor);
                return;
            }
            
            // Create a deep clone of the map to simulate the state
            Map simMap = map.createDeepClone();
            
            // Get all unit IDs from the actions
            HashSet<int> unitIds = new HashSet<int>();
            foreach (Action action in individual.actionSequence[horizon])
            {
                unitIds.Add(action.operationUnitId);
            }
            
            // Convert to list for random selection
            List<int> unitIdList = new List<int>(unitIds);
            
            // Randomly select a subset of units to replace actions for
            int unitsToReplace = Math.Max(1, unitIdList.Count / 3);
            HashSet<int> unitsToReplaceSet = new HashSet<int>();
            
            for (int i = 0; i < unitsToReplace && unitIdList.Count > 0; i++)
            {
                int index = rnd.Next(unitIdList.Count);
                unitsToReplaceSet.Add(unitIdList[index]);
                unitIdList.RemoveAt(index);
            }
            
            // Remove actions for the selected units
            individual.actionSequence[horizon].RemoveAll(a => unitsToReplaceSet.Contains(a.operationUnitId));
            
            // Generate new actions for these units
            List<Unit> unitsToAct = new List<Unit>();
            foreach (int unitId in unitsToReplaceSet)
            {
                Unit unit = simMap.getUnit(unitId);
                if (unit != null && !unit.isActionFinished() && unit.getTeamColor() == teamColor)
                {
                    unitsToAct.Add(unit);
                }
            ...
            // Generate and add new actions for these units
            foreach (Unit unit in unitsToAct)
            {
                // Decide whether to prioritize attack or movement
                bool prioritizeAttack = rnd.NextDouble() < ATTACK_BIAS;
                ...
                if (prioritizeAttack)
                {
                    ...
                    if (validAttackActions.Count > 0)
                    {
                        // Choose a random attack action
                        Action action = validAttackActions[rnd.Next(validAttackActions.Count)];
                        individual.actionSequence[horizon].Add(action);
                        simMap.executeAction(action);
                        continue;
                    }
                }
        ...
        // Modify parameters of existing actions
        private void ModifyExistingActions(Individual individual, int horizon, Map map, int teamColor)
        {
            // If there are no actions at this horizon, generate new ones
            if (individual.actionSequence[horizon].Count == 0)
            {
            ...
            foreach (Action action in individual.actionSequence[horizon])
            {
                ...
                // For move actions, potentially change the destination
                if (action.actionType == Action.ACTIONTYPE_MOVEONLY)
                {
                    // 50% chance to modify the destination
                    if (rnd.NextDouble() < 0.5)
                    {
                        ...
                        if (validPositions.Count > 0)
                        {
                            ...
                            Tuple<int, int> newPos = validPositions[rnd.Next(validPositions.Count)];
                            Action newAction = Action.createMoveOnlyAction(unit, newPos.Item1, newPos.Item2);
                            newActions.Add(newAction);
                            simMap.executeAction(newAction);
                            continue;
                        }
                    }
                }
                // For attack actions, potentially change the target
                else if (action.actionType == Action.ACTIONTYPE_MOVEANDATTACK)
                {
                    // 50% chance to modify the target
                    if (rnd.NextDouble() < 0.5)
                    {
                        List<Action> attackActions = RangeController.getAttackActionList(unit, simMap);
                        List<Action> validAttackActions = new List<Action>();
                        ...
                        {
                            // Choose a random attack action
                            Action newAction = validAttackActions[rnd.Next(validAttackActions.Count)];
                            newActions.Add(newAction);
                            simMap.executeAction(newAction);
                            continue;
                        }
                ...
                // If we didn't modify the action, keep the original
                newActions.Add(action.createDeepClone());
                simMap.executeAction(action);
            }
            
            individual.actionSequence[horizon] = newActions;
        }
