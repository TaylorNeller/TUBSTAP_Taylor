...
namespace SimpleWars
{
	...
	/// 製作者 Muto
    /// </summary>
    class AI_M_UCT_HR_RR_TT : Player
    {
        // パラメータ
        ...
        private static M_GameTree root;
        ...
        // ---------------------------------------------------------------------------
        //  Transposition‑table helpers (place near the top of AI_M_UCT_HR_TT class)
        // ---------------------------------------------------------------------------
        private static readonly ulong[] zobrist = InitZobrist();
        private static readonly Dictionary<ulong, TTEntry> TT = new Dictionary<ulong, TTEntry>();     // <hash,node>
        private const int ZOBRIST_SEED = 42;

        private static ulong[] InitZobrist()
        {
            const int SLOTS_PER_UNIT = 32;          // x,y,hp,team packed into one index
            const int MAX_UNITS      = 100;         // over‑allocate a bit
            var rnd = new Random(ZOBRIST_SEED);
            ulong[] tbl = new ulong[MAX_UNITS * SLOTS_PER_UNIT + 100]; // +turns
            for (int i = 0; i < tbl.Length; i++)
                tbl[i] = RandomU64(rnd);
            return tbl;
        }
        private static ulong RandomU64(Random r)
        {
            var buf = new byte[8];
            r.NextBytes(buf);
        ...
        /// <summary>Very quick 64‑bit hash of a Map position.</summary>
        private static ulong Hash(Map m)
        {
            ulong h = 0;

            // turn
            h ^= zobrist[90 * 32 + (m.getTurnCount() % 100)];

            // both armies
            List<Unit> all = m.getUnitsList(0,true,true,false);
            all.AddRange(m.getUnitsList(1,true,true,false));
            foreach (var u in all)
            {
                // Pack x,y,hp,team into one “slot” index to stay in‑bounds.
                int slot = (u.getID() * 32) +
                        ((u.getXpos() & 0xF)      ) +      // 0‑15
                        ((u.getYpos() & 0xF) << 4) +      // 0‑240
                        ((u.getHP()   & 0x7) << 8) +      // 0‑1792
                        ((u.getTeamColor() & 0x1) << 11); // 0‑2048
                h ^= zobrist[slot % zobrist.Length];
            }
            return h;
        }

        private sealed class TTEntry
        {
            public int    SimNum;
            public double TotalScore;
            public double LastScore;

            public double MeanScore => SimNum == 0 ? 0 : TotalScore / SimNum;
        }

        private static void CopyStats(TTEntry src, M_GameTree dst)
        {
            dst.simnum     = src.SimNum;
            dst.totalscore = src.TotalScore;
            dst.lastscore  = src.LastScore;
            dst.housyuu    = src.MeanScore;
        }
        ...
        public Action makeAction(Map map, int teamColor, bool turnStart, bool gameStart)
        {
            ...
            if (root == null || !MapsAreEquivalent(root.board, map))
            {
                TT.Clear();
        ...
        public static M_GameTree makeroot(Map map, int teamcolor)
        {
            ...
            ulong rootKey = Hash(map);
            root = new M_GameTree { board = map.createDeepClone() };
            TTEntry e;
            if (!TT.TryGetValue(rootKey, out e))
                TT[rootKey] = e = new TTEntry();
            else
                CopyStats(e, root);
        ...
        public static void search(M_GameTree n, int teamcolor)
        {
            if (n.next.Count == 0)
            {
                int sideToMove = n.board.getUnitsList(teamcolor, false, true, false).Count > 0
                                ? teamcolor
                                : getenemycolor(teamcolor);

                development(n, sideToMove);

                // if there are still no moves, treat the position as terminal
                if (n.next.Count == 0)
                {
                    n.simnum++;
                    n.lastscore = evaluateStateValue(n.board, teamcolor);
                    n.totalscore = n.lastscore;
                    n.housyuu = n.lastscore;
                    lastid = 0;
                    return;
                }
            }
            ...
            // -------------------------------------------------------------------
            //  after n.next[maxID] has been updated (simnum++, totalscore += …)
            // -------------------------------------------------------------------
            ulong posKey = Hash(n.next[maxID].board);
            if (TT.TryGetValue(posKey, out var tte))
            {
                tte.SimNum     = n.next[maxID].simnum;
                tte.TotalScore = n.next[maxID].totalscore;
                tte.LastScore  = n.next[maxID].lastscore;
            }
        ...
        public static void development(M_GameTree n, int teamcolor)
        {
            ...
            foreach (Action act in allActions)
            {
                var childMap = n.board.createDeepClone();
                if (n.board.getUnitsList(teamcolor,true,false,false).Count == 0)
                    childMap.incTurnCount();
                childMap.executeAction(act);

                ulong key = Hash(childMap);

                var child = new M_GameTree
                {
                    board = childMap,
                    act   = act,
                    depth = n.depth + 1
                ...
                if (TT.TryGetValue(key, out var entry))        // transposition hit → reuse stats
                    CopyStats(entry, child);
                else                                           // first time → create empty entry
                    TT[key] = new TTEntry();
