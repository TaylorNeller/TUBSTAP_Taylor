...
namespace SimpleWars {
    ...
    class AI_SatD2FULLORDER
        ...
        //0で1手番読み　1で深さ2手番読み
        const int DEFAULT_MAX_SEARCH_DEPTH = 0;

        const int MAX_ACTIVE_UNIT_friend = 5;
        const int MAX_ACTIVE_UNIT_enemy = 5;
        ...
        private SAT_UNIT_MOVE_ORDER[] moveOrderOptions = null;
        ...
        public string getName() {
            return "SatDmax"+DEFAULT_MAX_SEARCH_DEPTH+"_Order_FULL" + "AtkPrune-"+atkPrune+"_MvPrune-"+movePrune+
        ...
        private bool decidePrepareFriendRouteMap_TurnChange(int currentTurn) {
            return false;   //深さ2までの探索なら，ここはずっとFalseで良い．
        }
        ...
        public int depthFirstSearch(SatQuickMap map_parent, int num_MovedFriendUnits, 
            ...
            List<int> suggested_moves = new List<int>();
            foreach (SatQuickUnit u in units_unmoved) {
                suggested_moves.AddRange(suggestMoves(map_parent,u));
            }
            
            foreach (int selected_act in suggested_moves) {
                    
                if (debug_) {
                    SAT_FUNC.PRINT_ACTIONS(suggested_moves);
                        
                    SAT_FUNC.WriteLine("ExeAct->:" + SatQuickAction.toOneLineString(selected_act)); 
                }
                ...
                int childEval;
                if (changeTurn) {
                    childEval = -depthFirstSearch(map_parent, 0, currentTurn + 1, maxTurn, -beta, -alpha, inheretMoveOrder);
                    ...
                    childEval = depthFirstSearch(map_parent, num_MovedFriendUnits + 1, 
                        currentTurn, maxTurn, alpha, beta, inheretMoveOrder);
                }
